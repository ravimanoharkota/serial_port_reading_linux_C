...
struct serial_s *serial_create()
{
    //Allocate serial object.
    struct serial_s *s = malloc(sizeof(struct serial_s));
...
static int buffer_put(struct serial_s *s, char c)
{
...
static void serial_rx_callback(struct serial_s *s, char data[], int length)
{
...
int main(int argc, const char *argv[]) {
  ...
  if ( serial_port < 0 ) {
    printf("Error opening serial port fd");
    return -1;
  }
...
//  int num_bytes = read(serial_port, &read_buf, sizeof(read_buf));
 ...
 //Set up poll file descriptors.
    ufds.fd = serial_port;        //Attach socket to watch.
  ...
  while (1) {
	  ...
	  if ( poll_res > 0 ) {
	    ...
	    if( num_bytes > 0 ) {
	      ...
	      printf("%s", read_buf);
	      memset(&read_buf, '\0', sizeof(read_buf));
              serial_rx_callback(sport, (char *)read_buf, num_bytes);
	    }
	    else
	      printf("read failed");
